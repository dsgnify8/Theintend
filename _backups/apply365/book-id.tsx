...
import { applyNewTime, createBooking, getBookingById, useExpertBookings } from '@/lib/bookings';
...
export default function BookScreen() {
  ...
  const finalizeBooking = async (orderId?: string | null) => {
    ...
    if (isReschedule) {
      ...
      // Adds the new time to the calendar. The old event stays, because there
      // is no way to remove one yet.
      createCalendarEvent({
        expertId: String(id),
        summary: `${bookingTitle()} \u00B7 The Intend`,
        description: 'Moved through The Intend.',
        startIso: slot.toISOString(),
        endIso: new Date(slot.getTime() + (svc?.durationMin ? svc.durationMin : 60) * 60000).toISOString(),
