...
// Where the consultation link goes. Her booking page, so the free
// consultation sits alongside everything else she offers.
// Straight to the sixty minute initial consultation rather than her booking
// page in general, since that is the one that matters here. The booking
// screen takes a service parameter already.
export const HEALTH_PROGRAM_CONSULT_ROUTE =
  '/book/joanna-gudkina?service=joanna-gudkina-initial-consultation';
...
export const HEALTH_PROGRAMS: HealthProgram[] = [
  ...
  {
    id: 'acne-skin',
    title: 'Acne and Skin Restoration',
    focus: 'Skin, inflammation and the gut',
...
// The product id in App Store Connect. Derived from the program id so the two
// can never drift, and underscored because Apple does not take hyphens well.
export function productIdFor(programId: string): string {
  return `com.theintend.app.program.${programId.replace(/-/g, '_')}`;
}

export function programIdForProduct(productId: string): string | null {
  const tail = productId.split('com.theintend.app.program.')[1];
  if (!tail) return null;
  const guess = tail.replace(/_/g, '-');
  return HEALTH_PROGRAMS.some((p) => p.id === guess) ? guess : null;
}

export const ALL_PRODUCT_IDS = HEALTH_PROGRAMS.map((p) => productIdFor(p.id));
