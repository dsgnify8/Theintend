        ...
        {resume.length ? (
          <View style={[styles.band, styles.bandTint]}>
            ...
            <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.strip}>
              {resume.map((r) => {
                const src = typeof r.image === 'string' ? { uri: r.image } : r.image;
                if (!src) {
                  return (
                ...
                return (
                  <Pressable key={r.key} style={[styles.resumeCard, styles.resumeCover]} onPress={() => router.push(r.route as any)}>
                    <Image source={src} style={StyleSheet.absoluteFill} resizeMode="cover" />
                    <View style={[StyleSheet.absoluteFill, styles.resumeScrim]} />
                    <Text style={[styles.resumeKind, styles.resumeKindOn]}>{r.kind}</Text>
                    <Text style={[styles.resumeTitle, styles.resumeTitleOn]} numberOfLines={3}>{r.title}</Text>
                    <View style={{ flex: 1 }} />
                    <Text style={[styles.resumeMeta, styles.resumeMetaOn]}>{r.meta}</Text>
                  </Pressable>
                );
              })}
            </ScrollView>
...
const styles = StyleSheet.create({
  ...
  resumeCover: { overflow: 'hidden', borderColor: 'transparent' },
  resumeScrim: { backgroundColor: 'rgba(28,24,20,0.48)' },
  resumeKindOn: { color: 'rgba(255,255,255,0.92)' },
  resumeTitleOn: { color: '#FFFFFF' },
  resumeMetaOn: { color: 'rgba(255,255,255,0.82)' },
