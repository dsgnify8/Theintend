...
import { ActivityIndicator, Alert, Dimensions, Image, KeyboardAvoidingView, Modal, Platform, Pressable, ScrollView, StyleSheet, Text, TextInput, View, Linking } from 'react-native';
...
const YOU_SKY_H = Math.round(Dimensions.get('window').height * 0.55);
// A light image under dark type, so no scrim is needed. Only the fade.
const YOU_FADE = ['rgba(247,242,234,0.10)', 'rgba(247,242,234,0)', 'rgba(247,242,234,0.88)', '#F7F2EA'];
const YOU_STOPS = [0, 0.42, 0.85, 1];
  ...
  return (
    <View style={styles.safe}>
      <ScrollView contentContainerStyle={[styles.content, demo && { paddingBottom: 280 }]} showsVerticalScrollIndicator={false}>
        <View style={styles.youSky} pointerEvents="none">
          <Image source={YOU_SKY} style={StyleSheet.absoluteFill} resizeMode="cover" />
          <LinearGradient colors={YOU_FADE} locations={YOU_STOPS} style={StyleSheet.absoluteFill} />
        </View>
...
const styles = StyleSheet.create({
  ...
  youSky: { position: 'absolute', top: 0, left: 0, right: 0, height: YOU_SKY_H },
