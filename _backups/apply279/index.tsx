...
const BOOK_SHOT = require('@/assets/images/book-quiet-engine.jpg');
const SKY_FADE = [
  'rgba(28,24,20,0.42)',
  'rgba(28,24,20,0.16)',
  'rgba(247,242,234,0)',
...
const SKY_STOPS = [0, 0.18, 0.64, 0.93, 1];
...
export default function HomeScreen() {
  ...
  // Once answered, the card fades out and unmounts so the page moves up.
  ...
  const scheduleFade = () => {
    ...
    fadeTimer.current = setTimeout(() => {
      Animated.timing(cardOpacity, { toValue: 0, duration: 700, useNativeDriver: true }).start(({ finished }) => {
        if (finished) { setShowChips(false); setMoodDone(true); }
      });
    }, 6000);
  };
  ...
  return (
    <View style={styles.safe}>
      <ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
        ...
        {moodPicker.checked && moodPicker.visible && !moodDone ? (
        <Animated.View style={[styles.moodCard, { opacity: cardOpacity }]}>
        ...
        <Pressable style={styles.bookCard} onPress={() => router.push('/ebook/quiet-engine')}>
          <Image source={BOOK_SHOT} style={styles.bookImage} resizeMode="cover" />
          <View style={styles.bookBody}>
            <View style={styles.bookMetaRow}>
              <Text style={styles.bookTag}>GUT HEALTH</Text>
              <Text style={styles.bookTime}>50 min read</Text>
            </View>
            <Text style={styles.bookTitle}>The Quiet Engine</Text>
            <Text style={styles.bookBlurb}>What gut health actually means, and how far it reaches into mood, skin and hormones.</Text>
          </View>
        </Pressable>
...
const styles = StyleSheet.create({
  ...
  hero: { marginHorizontal: -20, paddingHorizontal: 20, paddingBottom: 62, marginBottom: 20, overflow: 'hidden' },
  ...
  moodCard: { backgroundColor: 'rgba(255,255,255,0.13)', borderRadius: 20, borderWidth: 1, borderColor: 'rgba(255,255,255,0.26)', padding: 18, marginBottom: 0 },
  ...
  featureHead: { fontFamily: FONT_SERIF, fontSize: 30, lineHeight: 36, color: COLORS.ink, marginTop: 4 },
  featureSub: { fontSize: 14, color: COLORS.muted, marginTop: 6, marginBottom: 16 },
  bookCard: { backgroundColor: COLORS.card, borderRadius: 22, borderWidth: 1, borderColor: COLORS.line, overflow: 'hidden', marginBottom: 34 },
  bookImage: { width: '100%', height: 190, backgroundColor: COLORS.accentSoft },
  bookBody: { padding: 20 },
  ...
  bookTitle: { fontFamily: FONT_SERIF, fontSize: 26, lineHeight: 32, color: COLORS.ink },
