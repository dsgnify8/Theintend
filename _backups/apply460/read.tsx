...
export default function LibraryScreen() {
  ...
  const shelfEbooks = useShelfEbooks();
  const ebooks = useMemo(
    () => shuffleToday(shelfEbooks.items),
    [shelfEbooks.items.length],
  );
...
function TitleCard({ item, uri, isAdmin }: { item: LibraryItem & { url?: string; coverUrl?: string }; uri?: string; isAdmin: boolean }) {
  const router = useRouter();
  const [busy, setBusy] = useState(false);
  const open = () =>
    router.push(item.pdf || item.html || (item as any).url ? `/ebook/${item.id}` : `/title/${item.id}`);
  return (
    <Pressable
    ...
    >
      {uri ? (
        ...
        <Image source={{ uri: (item as any).coverUrl }} style={styles.shelfCover} resizeMode="cover" />
      ) : item.cover ? (
