...
const BLANK = {
  ...
  // Arabic-only file and cover. Uploaded to a language-tagged storage
  // path so the two versions do not overwrite each other. When empty,
  // the English file/cover is shown to Arabic readers too.
  arFileUrl: '', arCoverUrl: '',
  i18nRaw: {},
};

// A new name every time, so a replaced file gets a new address and nothing
// serves the old one from a cache.
function pathFor(id: string, suffix: string, lang: Lang = 'en') {
  const langTag = lang === 'ar' ? '-ar' : '';
  return `${id || 'book'}${langTag}-${Date.now()}${suffix}`;
}
  ...
  const save = async () => {
    ...
    const nextI18n = {
      ...
      ar: {
        ...
        // Language-specific file and cover. Trimmed empty is stored as
        // empty string; the read path treats that as "no Arabic file"
        // and serves the English one.
        file_url: (arFileUrl ?? '').trim(),
        cover_url: (arCoverUrl ?? '').trim(),
      },
    };
        ...
        <ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false} keyboardShouldPersistTaps="handled">
          ...
          {form ? (
            <View style={styles.card}>
              ...
              {/* The book + cover pickers follow the active language tab.
                  When lang === 'ar' the upload targets arFileUrl / arCoverUrl
                  and the URL input/preview reflect the Arabic values. When
                  lang === 'en' the target is the English canonical field.
                  Arabic is optional; if the Arabic slot is empty the
                  reader shows the English file/cover to Arabic users. */}
              <Text style={styles.label}>The book ({lang === 'ar' ? 'Arabic' : 'English'})</Text>
              <View style={styles.pickRow}>
                ...
                {(lang === 'ar' ? form.arFileUrl : form.file_url) ? <Ionicons name="checkmark-circle" size={19} color={COLORS.taupeBlue} /> : null}
              </View>
              <TextInput
                value={lang === 'ar' ? (form.arFileUrl ?? '') : form.file_url}
                onChangeText={(t) => set(lang === 'ar' ? 'arFileUrl' : 'file_url', t)}
                placeholder={lang === 'ar' ? 'or paste a link to the Arabic html (leave empty to fall back to English)' : 'or paste a link to the html'}
                placeholderTextColor={COLORS.muted}
                autoCapitalize="none"
                autoCorrect={false}
                style={styles.input}
              />
              {lang === 'ar' ? <Text style={styles.note}>Optional. Leave empty to serve the English file to Arabic readers.</Text> : null}

              <Text style={styles.label}>Cover ({lang === 'ar' ? 'Arabic' : 'English'})</Text>
              <View style={styles.pickRow}>
                ...
                {(lang === 'ar' ? form.arCoverUrl : form.cover_url) ? (
                  <Image source={{ uri: lang === 'ar' ? form.arCoverUrl : form.cover_url }} style={styles.coverPeek} resizeMode="cover" />
                ) : null}
              </View>
              <TextInput
                value={lang === 'ar' ? (form.arCoverUrl ?? '') : form.cover_url}
                onChangeText={(t) => set(lang === 'ar' ? 'arCoverUrl' : 'cover_url', t)}
                placeholder={lang === 'ar' ? 'or paste a link to the Arabic cover (leave empty to fall back to English)' : 'or paste a link to the cover'}
                placeholderTextColor={COLORS.muted}
                autoCapitalize="none"
                autoCorrect={false}
                style={styles.input}
              />
              {lang === 'ar' ? <Text style={styles.note}>Optional. Leave empty to use the English cover on the Arabic shelf.</Text> : null}
