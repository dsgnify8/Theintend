...
export function fromRow(r: any): Ebook {
  ...
  return {
    ...
    color: r.color,
    // file_url and cover_url swap with locale the same way the text fields
    // do. When the app is in Arabic and i18n.ar.file_url is filled, that
    // wins; otherwise the English canonical url is used. Same for the
    // cover. This flows all the way to the reader through asShelfItem
    // without any change on the ebook viewer side.
    file_url: pick(r.file_url, ar.file_url),
    cover_url: pick(r.cover_url ?? null, ar.cover_url) ?? null,
...
export async function loadEbookForEdit(id: string): Promise<{
  en: Ebook;
  ar: { title: string; author: string; description: string; length: string; tag: string; read_time: string; file_url: string; cover_url: string };
  i18nRaw: any;
} | null> {
  try {
    const { data } = await supabase.from('ebooks').select('*').eq('id', id).maybeSingle();
    ...
    return {
      ...
      ar: {
        ...
        // Arabic-only file and cover, held under i18n.ar rather than a
        // separate column. Empty means fall back to the English file
        // when someone reads the book in Arabic.
