...
// Opened from the guide in notifications, never on its own.
const TOUR_STEPS: TourStep[] = [
  { key: 'tour-coming', title: 'What is booked', body: 'Sessions people have booked with you, ahead and behind. Tap one to add a link or an address.' },
  { key: 'tour-availability', title: 'When you are free', body: 'Set the hours you can be booked, and connect a calendar so nothing lands on top of something else.' },
  { key: 'tour-offerings', title: 'What you offer', body: 'Propose a new session or programme, or change the price of one you already have. Everything is approved before it goes live.' },
  { key: 'tour-money', title: 'What you are owed', body: 'What each session earns you, what has been sent, and where it went.' },
];
...
export default function ExpertPanel() {
  ...
  const { rects, setTarget, onTargetLayout, onScroll, focus, remeasure } = useTour(scrollRef);
  const [tourOpen, setTourOpen] = useState(tour === '1');
  ...
  return (
    <Screen router={router}>
      <ScrollView contentContainerStyle={styles.content} ref={scrollRef}
        ...
        onScrollEndDrag={remeasure} showsVerticalScrollIndicator={false}>
        {/* Compact header */}
        ...
        <View ref={setTarget('tour-coming')} onLayout={onTargetLayout('tour-coming')} collapsable={false} style={styles.tabRow}>
        ...
        <Pressable ref={setTarget('tour-availability')} onLayout={onTargetLayout('tour-availability')} style={styles.row} onPress={() => router.push('/expert-availability')}>
        ...
        <ActionRow
          ...
          targetRef={setTarget('tour-offerings')}
          onLayout={onTargetLayout('tour-offerings')}
        ...
        <ActionRow
          ...
          targetRef={setTarget('tour-money')}
          onLayout={onTargetLayout('tour-money')}
        />
      </ScrollView>
    </Screen>
  );
...
function Screen({ children, router }: { children: any; router: any }) {
  return (
    <SafeAreaView style={styles.safe} edges={['top']}>
      ...
      {tourOpen ? (
        <Tour steps={TOUR_STEPS} rects={rects} focus={focus} onDone={() => setTourOpen(false)} />
      ) : null}
    </SafeAreaView>
  );
}
