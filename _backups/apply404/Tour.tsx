...
// Give it the screen's ScrollView and it drives the scroll itself.
//
// The order matters. It asks the row where it is on screen now, scrolls by the
// difference between that and where it should sit, then measures again once
// the screen has stopped. Scrolling to a remembered position instead was what
// kept putting the ring in the wrong place.
export function useTour(scrollRef?: any) {
  ...
  // Only a fallback now, for a row that has never been on screen.
  ...
  const onScroll = (e: any) => {
    ...
    if (typeof y === 'number') offset.current = y;
  };

  const store = (key: string, x: number, y: number, w: number, h: number) => {
    setRects((prev) => {
      const old = prev[key];
  ...
  const measure = (key: string) => {
    ...
    node.measureInWindow((x: number, y: number, w: number, h: number) => {
      if (w && h) store(key, x, y, w, h);
    });
  };

  const focus = (key: string) => {
    active.current = key;
    // Clear the old position so a wrong ring is never shown while settling.
    setRects((prev) => (prev[key] ? { ...prev, [key]: undefined } : prev));

    const { height } = Dimensions.get('window');
    const rest = height * REST;
    const sv = scrollRef?.current;
    const node = nodes.current[key];

    const settle = () => {
      setTimeout(() => measure(key), 380);
      setTimeout(() => measure(key), 760);
    };

    const byLayout = () => {
      const ly = ys.current[key];
      if (sv?.scrollTo && typeof ly === 'number') {
        sv.scrollTo({ y: Math.max(0, ly - rest), animated: true });
      }
      settle();
    };
    ...
    if (node && typeof node.measureInWindow === 'function') {
      node.measureInWindow((x: number, y: number, w: number, h: number) => {
        if (!w || !h) { byLayout(); return; }
        // How far it is from where it should be, from where we are now.
        const delta = y - rest;
        if (sv?.scrollTo && Math.abs(delta) > 2) {
          sv.scrollTo({ y: Math.max(0, offset.current + delta), animated: true });
          settle();
        } else {
          store(key, x, y, w, h);
          settle();
  ...
  // Called by the screen whenever scrolling stops. Only the step being shown,
  // since the others are not on screen to measure.
  const remeasure = () => {
    if (active.current) measure(active.current);
  };

  return { rects, setTarget, onTargetLayout, onScroll, focus, remeasure };
}
