...
// What the scroll shows: the pool first, then anything generated for them.
// A pool line the person has already saved a copy of is dropped, so it does
// not appear twice.
export async function loadFeed(userId: string, category: string): Promise<Affirmation[]> {
  ...
  const pool: Affirmation[] = lib
    .filter((l) => !mineTexts.has(l.text))
    .map((l) => ({ id: LIB_PREFIX + l.id, category, text: l.text, liked: false }));
  return [...shuffleFor(userId + ':' + category, pool), ...mine];
}
