...
// Sends what is owed on programs.
//
// The payout row first, then the stamp, and the row is removed if the stamp
// fails. Same order and same reason as recordPayout: a sale must never be able
// to end up paid twice because one write of the two succeeded.
export async function recordProgramPayout(opts: {
  expertId: string;
  sales: ProgramSale[];
  reference?: string | null;
}): Promise<{ error: any }> {
  const payable = opts.sales.filter((s) => isReady(s));
  if (!payable.length) return { error: { message: 'Nothing is ready to pay yet.' } };

  const amount = payable.reduce((n, s) => n + (s.expert_share_minor ?? 0), 0);
  ...
  try {
    const { data: po, error } = await supabase
      ...
      .select('id')
      .single();
    if (error || !po) return { error: error ?? { message: 'Could not record that.' } };

    const { error: stampError } = await supabase
      .from('program_purchases')
      .update({ payout_id: (po as any).id })
      .in('id', payable.map((s) => s.id));

    if (stampError) {
      // Take the payout back out, or these sales stay payable while a payout
      // row says they were paid.
      await supabase.from('payouts').delete().eq('id', (po as any).id);
